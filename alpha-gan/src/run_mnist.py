...
def build_input_pipline(dataset, batch_size, heldout_size):
  """Build an Iterator switching between train and heldout data."""
  
  # Build a iterator over the training set that batch size is batch_size.
  training_dataset  = dataset.repeat().batch(batch_size)
  training_iterator = training_dataset.make_one_shot_iterator()

  # Build a iterator over the heldout set that batch size is heldout_size.
  heldout_dataset  = dataset.take(heldout_size).repeat().batch(heldout_size)
  heldout_iterator = heldout_dataset.make_one_shot_iterator()
...
def main():
  ...
  with tf.Graph().as_default():
    ...
    # Add a variation initializer Op.
    ...
    with tf.Session() as sess:
      # Create a FileWriter to output summaries and and the graph.
